...
//change the continuity of the track
void changeContinuity(Fl_Widget*, TrainWindow* tw)
{
	
	if (tw->splineBrowser->selected(1)){
		tw->world.continuity = 1;
		if (tw->runButton->value() == 0){
			tw->world.angle = 45.0228;
	...
	if (tw->splineBrowser->selected(3)){
		tw->world.continuity = 3;
		if (tw->runButton->value() == 0){
			tw->world.angle = -13.3905;
			tw->world.xaxis = -0.05;
			tw->world.yaxis = 5.00;
			tw->world.zaxis = 33.33;
		}
	}
	tw->damageMe();
}
...
//change the model
void changeModel(Fl_Widget*, TrainWindow* tw){
	if (tw->modelBrowser->selected(1)){
		tw->world.model = 1;
	}
...
//callback that change track
void changeTrack(Fl_Widget*, TrainWindow* tw){
	if (tw->trackBrowser->selected(1)){
		tw->world.trackType = 1;
	}
	if (tw->trackBrowser->selected(2)){
		tw->world.trackType = 2;
	}
	if (tw->trackBrowser->selected(3)){
		tw->world.trackType = 3;
	}
